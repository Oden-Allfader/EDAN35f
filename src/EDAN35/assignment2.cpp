...
edan35::Assignment2::run()
{
	...
	auto const quad_shape = parametric_shapes::createQuad(10, 10, 1000, 1000);
	Node quad;
	...
	mCamera.mMouseSensitivity = 0.006f;
	mCamera.mMovementSpeed = 0.5f;
	...
	auto const fire_texture						   = eda221::loadTexture2D("noise.png");
	quad.add_texture("diffuse_texture", fire_texture);
	quad.add_texture("Specular_texture", fire_texture);
	quad.rotate_x(bonobo:: pi*5/4);
	quad.set_translation(glm::vec3(0, 0, 50));
	...
	while (!glfwWindowShouldClose(window->GetGLFW_Window())) {
		...
		for (size_t i = 0; i < constant::lights_nb; ++i) {
			...
			quad.render(light_matrix, glm::mat4(), fill_gbuffer_shader, set_uniforms);
